...
def plot_samples_grid(z_df):
    out = Path("plots_chromosome")
    out.mkdir(exist_ok=True)

    if "mid" not in z_df.columns:
        z_df = z_df.copy()
        z_df["mid"] = z_df["start"] + 500_000

    chrom_order = [f"chr{i}" for i in range(1, 23)] + ["chrX", "chrY"]

    # исключаем служебные колонки
    meta_cols = {"chr", "start", "mid", "genome_pos"}
    sample_cols = [c for c in z_df.columns if c not in meta_cols]
    ...
    for sample in sample_cols:
        ...
        out_file = out / f"{sample}_grid.png"
...
def plot_chrX_boxplot(z_df):

    x_df = z_df[z_df["chr"].isin(["chrX", "X"])]

    meta_cols = {"chr", "start", "mid", "genome_pos"}
    sample_cols = [c for c in z_df.columns if c not in meta_cols]

    data = [x_df[s].dropna() for s in sample_cols]

    plt.figure(figsize=(12, 6))
    plt.boxplot(data, labels=sample_cols, showfliers=False)
    ...
    plt.ylabel("Z-score (chrX)")
    plt.title("chrX Z-score distribution per sample")

    plt.tight_layout()
    plt.savefig("chrX_boxplot.png", dpi=150)
    plt.close()
...
def main():
    ...
    plot_samples_grid(z_df)

    plot_chrX_boxplot(z_df)
